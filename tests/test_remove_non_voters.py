...
from telegram_automations.commands.remove_non_voters import (
    ServerClock,
    configure_parser,
...
from telegram_automations.common import AppConfig
from telegram_automations.polls.cleanup import CleanupError, Exclusions
from telegram_automations.runtime import CommandRuntime
...
class ArgumentValidationTests(unittest.TestCase):
    def test_input_defaults_to_non_voters_json_and_can_be_overridden(self) -> None:
        self.assertEqual(parse_args([]).input, Path("non_voters.json"))
        self.assertEqual(
            parse_args(["--input", "custom.json"]).input,
            Path("custom.json"),
        )

    def test_limit_caps_the_batch_and_batch_size_remains_an_alias(self) -> None:
        self.assertEqual(parse_args([]).batch_size, 10)
        self.assertEqual(parse_args(["--limit", "1"]).batch_size, 1)
        self.assertEqual(
            parse_args(["--batch-size", "2"]).batch_size,
            2,
        )
...
class RemovalLogTests(unittest.TestCase):
    def test_make_log_record_copies_identity_context_status_and_details(self) -> None:
        ...
        with patch("telegram_automations.commands.remove_non_voters.utc_now_iso", return_value="2026-08-01T12:00:00+00:00"):
    ...
    def test_make_log_record_omits_empty_details(self) -> None:
        with patch("telegram_automations.commands.remove_non_voters.utc_now_iso", return_value="now"):
            record = make_log_record(
                document={
                    "exported_by_user_id": 900,
                    "chat": {"id": -100123},
...
class ServerClockTests(unittest.IsolatedAsyncioTestCase):
    async def test_server_clock_uses_telegram_time_and_suspend_aware_elapsed(
        ...
        with patch(
            "telegram_automations.commands.remove_non_voters.suspend_aware_seconds",
    ...
    def test_server_clock_rejects_elapsed_clock_divergence(self) -> None:
        ...
        with (
            patch("telegram_automations.commands.remove_non_voters.suspend_aware_seconds", return_value=106.0),
    ...
    def test_finite_ban_window_must_still_have_five_minutes(self) -> None:
        ...
        with patch("telegram_automations.commands.remove_non_voters.suspend_aware_seconds", return_value=100.0):
            ensure_fresh_finite_ban_window(
...
class DryRunTests(unittest.IsolatedAsyncioTestCase):
    async def test_run_dry_run_never_kicks_or_sends_edit_banned(self) -> None:
        ...
        runtime = CommandRuntime(
            config_dir=Path("."),
            config=AppConfig(1, "hash", Path("unused-session")),
            client=client,
            me=SimpleNamespace(id=900, bot=False),
        )

        with (
            patch("telegram_automations.commands.remove_non_voters.load_export_document", return_value=document),
            patch(
                "telegram_automations.commands.remove_non_voters.load_poll_context",
                new=AsyncMock(return_value=context),
            ),
            patch("telegram_automations.commands.remove_non_voters.load_pending_kicks", return_value=([], [])),
            patch(
                "telegram_automations.commands.remove_non_voters.fetch_voter_ids",
                new=AsyncMock(return_value=set()),
            ),
            patch(
                "telegram_automations.commands.remove_non_voters.fetch_all_participants",
                new=AsyncMock(return_value=[user]),
            ),
            patch("telegram_automations.commands.remove_non_voters.print_candidate_table"),
    ...
    async def test_execute_supergroup_uses_one_finite_server_timed_ban(self) -> None:
        ...
        with (
            ...
            patch(
                "telegram_automations.commands.remove_non_voters.fetch_server_clock",
            ...
            patch("telegram_automations.commands.remove_non_voters.append_private_jsonl") as append_log,
            patch("telegram_automations.commands.remove_non_voters.print_candidate_table"),
            patch("telegram_automations.commands.remove_non_voters.sys.stdin.isatty", return_value=True),
