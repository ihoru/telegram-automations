...
ERROR_POLICY = ErrorPolicy(
    ...
    system_error=(
        "System or network error: {error}. Before retrying, always run a dry run; "
        "the log may contain a pending operation."
    ),
)
...
def configure_parser(parser: argparse.ArgumentParser) -> None:
    parser.description = (
        ...
        "group without a permanent ban. Without --execute, only a dry run is "
        "performed."
    )
    parser.add_argument(
        "--input",
        ...
        help=(
            "JSON produced by poll list-non-voters "
            "(default: non_voters.json)"
        ),
    )
    parser.add_argument(
        "--exclusions",
